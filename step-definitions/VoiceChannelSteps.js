...
When('user click on transfer call button in {string} window', async (session) => {
  await voiceChannel.transferCall(session);
});

When('user select the {string} transfer option in {string} window', async (transferType, session) => {
...
When('user transfers call to {string} queue in {string} window', async (queueName, session) => {
  await voiceChannel.transferCallToQueue(queueName, session);
});

When('user makes {string} of call in {string} window', async (transferType,session) => {
